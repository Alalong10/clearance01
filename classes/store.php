...
public static function saveStudent(){
	...
	if($existCheck = self::existOne('account_studentprofile', 'matric', $matric)==0){
		$now = self::CreatedOn();
		$stmt = $conn->db->prepare("INSERT INTO account_studentprofile (fullname, matric, password, course_id) VALUES (:fullname, :matric, :password, :course )");
...
public static function saveCourse(){
	...
	if($existCheck = self::existOne('system_coursedata', 'course_name', $course)==0){
		$now = self::CreatedOn();
		$stmt = $conn->db->prepare("INSERT INTO system_coursedata (course_name) VALUES (:course)");
		$stmt->bindParam(':course', $course, PDO::PARAM_STR);
		if ($stmt->execute()): return 1; else: return 0;	endif;
	}else {
		return 2;
...
public static function saveSubject(){
	...
	if($existCheck = self::existOne('system_subjectdata', 'name', $name)==0){
		$now = self::CreatedOn();
		$stmt = $conn->db->prepare("INSERT INTO system_subjectdata (curriculum_id, name, code) VALUES (:curriculum_id, :name, :code)");
...
public static function saveTeacher(){
	$conn = Database::getInstance();
	$name = $_POST['name'];
	$prof_type = $_POST['prof_type'];

	if($existCheck = self::existOne('system_teacher', 'name', $name)==0){
		$now = self::CreatedOn();
		$stmt = $conn->db->prepare("INSERT INTO system_teacher (name, prof_type) VALUES (:name, :prof_type)");
		$stmt->bindParam(':name', $name, PDO::PARAM_STR);
		$stmt->bindParam(':prof_type', $prof_type, PDO::PARAM_STR);
		if ($stmt->execute()): return 1; else: return 0;	endif;
	}else {
		return 2;
	}
...
public static function saveCurriculum(){
	$conn = Database::getInstance();
	...
	if($existCheck = self::existOne('system_curriculum', 'curriculum_name', $curriculum_name)==0){
		$now = self::CreatedOn();
		$stmt = $conn->db->prepare("INSERT INTO system_curriculum (course_id, curriculum_name, school_year, semester) VALUES (:course_id, :curriculum_name, :school_year, :semester)");
		$stmt->bindParam(':course_id', $course_id, PDO::PARAM_INT);
		$stmt->bindParam(':curriculum_name', $curriculum_name, PDO::PARAM_STR);
...
public static function getCourseList(){
	$conn = Database::getInstance();
	$select = $conn->db->prepare("SELECT * FROM system_coursedata");
	$select->execute();
	return $select;
}

public static function getCurriculumList(){
	$conn = Database::getInstance();
	$select = $conn->db->prepare("SELECT * FROM system_curriculum");
	$select->execute();
	return $select;
}
